...
def test_time_travel_loop_basic_timeout():
    ...
    tloop.run_until_complete(
        asyncio.wait_for(inner_cor(), timeout=total_time + 1, loop=tloop)
    )
...
def test_time_travel_loop_concurrent_sleep():
    ...
    async def inner_cor():
        ...
        task1 = asyncio.ensure_future(
            asyncio.sleep(delay=SLEEP_TIME * 3, loop=tloop), loop=tloop
        )
        task2 = asyncio.ensure_future(
            asyncio.sleep(delay=SLEEP_TIME * 1, loop=tloop), loop=tloop
        )
        task3 = asyncio.ensure_future(
            asyncio.sleep(delay=SLEEP_TIME * 4, loop=tloop), loop=tloop
        )
        task4 = asyncio.ensure_future(
            asyncio.sleep(delay=SLEEP_TIME * 2, loop=tloop), loop=tloop
        )
...
def test_time_travel_loop_complex_order():
    ...
    async def inner_cor():
        ...
        asyncio.ensure_future(
            asyncio.sleep(delay=500, loop=tloop), loop=tloop
        ).add_done_callback(
            lambda x: add_res(2)
        )  # time=0 --> 500

        asyncio.ensure_future(
            asyncio.sleep(delay=1500, loop=tloop), loop=tloop
        ).add_done_callback(
            lambda x: add_res(4)
        )  # time=0  --> 1500
